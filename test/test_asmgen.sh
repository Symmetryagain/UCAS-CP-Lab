...
for file in "${files[@]}"; do
    ...
    # 1. Compile CACT to RISC-V assembly
    echo "  1. Compiling to assembly..."
    if ! "$COMPILER" "$file" -o "$asmfile" 2>"$compile_err"; then
    ...
    "$COMPILER" "$file" --emit-IR -o "$irfile" 2>/dev/null || true
